...
### 2.5. Native (Capacitor iOS) — M_EXPANSION.S.64

```bash
pnpm build                 # vite build (base "/")
pnpm exec cap add ios      # ONE-SHOT: generate ios/ Xcode scaffold (needs Xcode)
pnpm cap:sync              # npx cap sync ios (after each build)
pnpm exec cap run ios      # launch on simulator or device
```

Or via Xcode:
```bash
cd ios/App && xed .        # opens ios/App/App.xcworkspace in Xcode
# Product → Run targets the active simulator or attached device.
```

iOS runtime config block in `capacitor.config.ts`:

```typescript
ios: {
  webContentsDebuggingEnabled: false, // match Android release default
  contentInset: 'automatic',          // WKWebView handles safe areas
}
```

`@capacitor/ios@8.3.4` is in devDependencies (pinned to the
same Capacitor major as core/cli/android). The `ios/` scaffold
directory is NOT checked in — it's generated by `pnpm exec cap
add ios` on each developer's machine; the build artifact (.ipa)
is produced via Xcode Archive or a future GitHub Actions
workflow with a macOS runner + the Apple Developer Program
provisioning profile.

The SQLCipher path (M_SEC.4) binds to iOS Keychain via the
`@capacitor/preferences` storage backend — same per-install
key bootstrap that already works on Android via
