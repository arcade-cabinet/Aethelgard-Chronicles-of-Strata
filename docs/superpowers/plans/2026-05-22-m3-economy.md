...
**Goal:** Build the autonomous Warcraft economic loop — resource nodes on the board, peons that harvest/carry/deposit without player intervention, a Town Hall, build mode for Farms and Barracks, a supply system, HUD resource counters, and the unit selection ring.
...
When a CARRYING peon is adjacent to the Town Hall, its load is added to the global
...
git commit -m "feat(ecs): add deposit system — CARRYING peon unloads at Town Hall"
```
...
no path gets a path to the Town Hall. This is what makes the loop autonomous.
  ...
  world.query(AssignedJob, HexPosition, PathQueue, Carrier).updateEach(([job, hex, path]) => {
    ...
    if (job.state === 'SEEKING') {
      ...
      if (path.steps.length === 0) {
        ...
        for (const adj of graph.get(peonKey) ? [job.targetKey] : []) void adj;
...
**Files:**
- Modify: `src/game/game-state.ts` (add economy, resource entities, Town Hall, second peon)
...
`startGame`: create the economy via `createEconomy()`; pick the Town Hall tile
...
spawn a second peon adjacent to the Town Hall; add `Harvester`, `Carrier`,
...
Renders resource nodes (tree/rock/gold GLB props from the manifest), the Town Hall
...
Render the Town Hall and every `Building` entity from the ECS via `assets.url` of a
...
the Hexagon Kit (`building-castle` for Town Hall, `building-farm`, `building-tower`
...
(trees, rocks, gold) must be visible on the board, the Town Hall must be visible at
...
carry it to the Town Hall, and the wood counter tick up. Screenshot. The autonomous
