...
| 05-build-menu-open | S4 build flow | **MISS** — `aethelgard:open-build-menu` dispatched but the menu DOM didn't appear in the shot. The BuildMenuButton requires a Palace selection first; the dispatch path needs that prerequisite. Filed: M_V11.POLISH.BUILD-MENU-CTA. |
...
| 09-procedural-buildings-zoom | S4 v0.11 — procedural Palace | **MISS** — `aethelgard:focus-palace` is NOT wired (grep src/ returns zero non-test hits). Filed: M_V11.POLISH.JOURNEY-CAMERA-EVENTS. |
...
1. **M_V11.POLISH.BUILD-MENU-CTA** — build menu DOM needs to
   appear either (a) when no Palace selected, drop a hint
   "Tap your Palace, then Build" or (b) auto-select Palace
...
3. **M_V11.POLISH.JOURNEY-CAMERA-EVENTS** — wire the focus-
   palace + zoom-to + pan-to-tile events so the journey
