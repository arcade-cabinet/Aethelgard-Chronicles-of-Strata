...
- [ ] M_V8.WONDER-TIMERS.N-PLAYER — `wonderTimers: Record<Faction, number>` widened to
...
- [ ] M_V8.FACTION-CAST-DEBT — Eliminate `as 'player' | 'enemy'` bridge casts in:
...
- [ ] M_V8.OUTLINE.CANVAS-MOUNT — `UnitHexOutline` + `BuildingOutlineRing` imported and mounted
...
- [ ] M_V8.DIFFICULTY-MULTIPLIER.N-PLAYER — `spawnIntervalFor(difficulty)` and
...
- [ ] M_V8.PORTAL-STONE.AUDIO — `useAudio.ts` (or `sound-map.ts`): add a bespoke
...
- [ ] M_V8.PORTAL-STONE.COOLDOWN-HOOK — `pathFollowSystem`: on each step arrival, if the arrived
...
- [ ] M_V8.NEWGAMEMODAL.N-PLAYER-PICKER — Replace the two-slot (player + enemy) color-picker
...
- [ ] M_V8.AI.DIPLO-EVALUATOR — `src/ai/evaluators/diplomatic.ts` ships
...
- [ ] M_V8.CI.VISUAL-BATTERY-HOOK — `.github/workflows/ci.yml` gains a new job
...
- [ ] M_V8.TUTORIAL.N-PLAYER-MODE — `OnboardingOverlay.tsx`: detect when `game.factions.length > 2`
...
- [ ] M_V8.PERF.PROFILE-PASS — Chrome performance trace via chrome-devtools-mcp (or Playwright
...
- [ ] M_V8.REVIEWER.FULL-CYCLE — dispatch `comprehensive-review:full-review` against
...
- [ ] M_V8.PARKING-LOT.V06 — drain any `[WAIT]` items from the v0.5/v0.6 parking lot that are
