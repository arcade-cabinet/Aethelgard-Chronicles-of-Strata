...
- [ ] [WAIT-DEPS] [HIGH] M_POLISH2.RTS.23 — Sentry tower: blocked on the RTS.22 Scout work (in flight via background agent — currently editing building-profiles.ts + economy-rules.ts). Promote when scout agent commits; RTS.23 is then a small BuildingProfile addition.
