...
- [ ] M_HUD.SHELL.7b — TOKEN SWEEP (TypeScript constants): migrate `src/hud/hud-theme.ts` `HUD_THEME.color.*` consumers (~200 inline-style components) to semantic CSS-var reads via `getComputedStyle()` helpers OR refactor each component to Tailwind utilities and remove the constant.
...
- [ ] M_HUD.SHELL.10b — Primitive adoption across the remaining 4 pages: NewGameModal (SectionCard → primitive), OnboardingOverlay (HeroBanner + StepProgressDots + TreasureButton), GameOverModal (Halo + TreasureButton), SystemMenu (IconButton). Remove the per-page reimplementations.
