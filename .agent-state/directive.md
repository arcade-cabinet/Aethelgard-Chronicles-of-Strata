...
- [ ] [HIGH] M_POLISH2.MOBILE.2 — ResourceBar portrait overflow on 360px width: today the 5-column grid (Wood/Stone/Gold/Science/Supply) wraps to 2 lines on narrow viewports. Switch to a horizontal scroll-snap row (`overflow-x: auto; scroll-snap-type: x mandatory`) so each chip occupies one viewport-width snap point, with a faint right-edge gradient cueing scrollability. Keep the 5-up grid for ≥420px. Cite spec 90-ui-hud.md §Resource Panel.
...
- [ ] [HIGH] M_POLISH2.MOBILE.6 — Tap-to-deselect zone: on mobile, tapping empty terrain inside the camera-rotate gesture often re-selects rather than deselects because the pointer-up dispatches both. Add a 6px movement-threshold check in `src/world/TileInteraction.tsx` `onPointerUp` so movements >6px are treated as camera-pan-end, not tile-select. Confirm via Playwright mobile test that a drag-pan does not select a tile.
- [ ] [HIGH] M_POLISH2.MOBILE.7 — Settings modal one-handed reachability: the SettingsModal close X is top-right and unreachable on a 6.7" portrait phone in single-thumb grip. Move primary controls (Done / Reset) to a bottom action bar with `env(safe-area-inset-bottom)` honoured; keep X for desktop. Visual baseline at 412×915.
...
- [ ] [HIGH] M_POLISH2.MOBILE.14 — SpeedControl + PauseControl unified mobile pill: today they're two separate floating chips that compete for space. Combine into a single pill `[⏸ ▷ ▷▷ ▷▷▷]` (pause / 1x / 2x / 3x) with the active speed highlighted. One tap to swap. Desktop keeps the split.
- [ ] [HIGH] M_POLISH2.MOBILE.15 — Resign + Settings + Credits collapse into mobile system-menu: on portrait viewports collapse ResignButton + SettingsModal trigger + CreditsModal trigger into a single top-left hamburger that opens a Radix `DropdownMenu`. Frees up the top bar for game-status (turn count, era, score). Visual baseline closed + open at 412×915.
