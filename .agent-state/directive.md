...
- [ ] [HIGH] M_POLISH3.SCENE.4 — Game-over UI doesn't trigger on `__game.outcome = 'win'/'loss'` mutation. Direct proxy mutation skips React state subscription so the WinScreen/LossScreen components don't re-render. Add a `window.__triggerGameOver(outcome)` that dispatches a CustomEvent the GameSession listens for + flushes the outcome through the normal state-update path.
