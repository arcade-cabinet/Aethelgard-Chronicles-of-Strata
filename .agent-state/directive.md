...
- [ ] M_HUD.SHELL.14b — Migrate `tests/e2e/*.spec.ts` + `tests/browser/*.test.tsx` HUD touchpoints to query-by-aria-label / by-testid + click/tap. Drop keyboard-driven test paths (kbd nav is now desktop-subpackage only).
