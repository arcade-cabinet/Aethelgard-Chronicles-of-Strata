...
- [ ] [HIGH] M_POLISH2.MOBILE.4a — NewGameModal Radix Tabs restructure: when product copy for the 3-tab labels (Mode / Realm / Loadout) is finalised, restructure NewGameModal as Radix Tabs. Each tab fits in one viewport without scroll on Pixel-7 portrait. Spec 10-player-journey.md §S2.
...
- [ ] [HIGH] M_POLISH2.MOBILE.12a — Onboarding copy rewrite (touch-first): rewrite each step's body text to illustrate the touch gesture appropriate to that lesson (single-tap to select; long-press for context; pinch to zoom; tap-and-hold to drag-pan). Today's copy reads as desktop-mouse instructions ("RIGHT-CLICK any tile to issue a move order"). Requires product copy lock-in.
- [ ] [HIGH] M_POLISH2.MOBILE.12b — Onboarding swipe-back-to-revisit: add a swipe-right gesture that returns to the previous step. Today only Next/Skip exists; a player who advanced too fast must Skip + restart. Spec a swipe-detector hook in OnboardingOverlay.
...
- [ ] [HIGH] M_POLISH2.MOBILE.13a — Tablet HUD slot resize: HudPill SLOT_POSITIONS gain a 'tablet' row alongside landscape + portrait. Wider chips + tighter row spacing to take advantage of the extra width.
- [ ] [HIGH] M_POLISH2.MOBILE.13b — Tablet side-docked SelectionPanel: when viewport.class === 'tablet', dock SelectionPanel to the right side at 240px wide (instead of the bottom 60vh band) so the canvas stays unobscured.
- [ ] [HIGH] M_POLISH2.MOBILE.13c — Tablet inline DiscoveriesPanel: render DiscoveriesPanel as an inline-right-side panel rather than a modal overlay on tablet, matching the side-docked SelectionPanel pattern.
- [ ] [HIGH] M_POLISH2.MOBILE.13d — Tablet Minimap 240×240: bump compact minimap from 96×96 to 240×240 in the tablet branch (more screen real estate available).
...
- [ ] [HIGH] M_POLISH2.RTS.24a — Wire RTS.24 costOf callback at every findPath call site. Audit src/ai/, src/game/commands.ts, src/ecs/systems/path-follow.ts; thread a closure `(key) => moveCostFor(game.board.tiles.get(key)?.type ?? 'GRASS')` into every findPath invocation. After wiring run economy-integration tests to confirm AI still pathfinds successfully + economy still meaningfully advances.
...
- [ ] [WAIT-DEPS] [HIGH] M_POLISH2.X4.31 — Espionage / scout-sight: blocked on M_POLISH2.RTS.22 Scout unit (in flight via background agent). Promote when Scout lands.
...
- [ ] [WAIT-DEPS] [HIGH] M_POLISH2.X4.38 — Counter-intel Watchhouse: blocked on RTS.22 Scout (needs scouts existing to spot them) + X4.27a Bronze-era gate. Promote when both land.
...
- [ ] [HIGH] M_POLISH2.MODES.41a — long-reign escalation events: in long-reign mode the existing random-events system fires 3 additional escalations every 5min (raiders, weather, tribute demand). Tap the random-events registry, add a mode-gated injector that schedules at 5/10/15/20/... min marks.
- [ ] [HIGH] M_POLISH2.MODES.41b — long-reign GameOverModal score sheet: "Survived 47:23 — Defeated 4 raids — Built 12 structures" copy when game.mode === 'long-reign'. GameOverModal already shows generic stats; this swaps in the long-reign-specific narrative line.
...
- [ ] [HIGH] M_POLISH2.MODES.42b — strata-wars tile-flip red-pulse VFX: when a player-controlled tile flips to enemy, fire a screen-edge red pulse via CriticalWarning + a louder horn. Already mostly wired in strata-wars mode — confirm and tune the volume/decay.
...
- [ ] [HIGH] M_POLISH2.MODES.43a — age-of-strata Renaissance+Wonder win condition: in age-of-strata mode, when the player reaches Renaissance AND has at least one complete Wonder building, flip outcome → 'win'. Already-existing wonderTimers system mirrors the wonder-completion check; add an age-of-strata gated branch that fires immediately on the conjunction instead of waiting the 300s WONDER_COUNTDOWN.
...
- [ ] [HIGH] M_POLISH2.MODES.44a — coexistence "Settle here" chip: tap Settlers in coexistence mode → tap empty tile → spawns new TownHall + faction-base anchor at that tile. Standard modes still seed bases at startGame; coexistence becomes the multi-base sandbox.
- [ ] [HIGH] M_POLISH2.MODES.44b — screenshot button bottom-right (coexistence only): captures the live r3f canvas via canvas.toDataURL or chrome offscreen-canvas → downloads as Aethelgard-{seed}-{timestamp}.png. Lets players share diorama state on social media.
