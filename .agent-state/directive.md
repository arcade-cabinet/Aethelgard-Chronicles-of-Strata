...
- [ ] [HIGH] M_BRAND.1 — game-mode names + UI must be brand-aligned, not POC leftovers. Today's keys/labels are generic ("red-vs-blue", "skirmish", "classic-rts", "4x", "coexist") — rename to Aethelgard-lore names that tell the player what FEEL they're choosing (e.g. "Border Clash" = quick balanced 1v1, "Frontier Raid" = pure-noise skirmish, "Long Reign" = endless invulnerable, "Strata Wars" = continent classic, "Age of Strata" = 4X turn-based, "Coexistence" = no-win sandbox). Update GameMode type, MODE_PRESETS keys, NewGameModal labels + hints, and the migration framework (snapshot-migration.ts: old keys → new keys for save-load compat). One PR.
- [ ] [HIGH] M_BRAND.2 — UI must surface mode as a PRESET BUNDLE, not just an axis. Each mode card in NewGameModal needs to render: a 1-line lore tagline, the bundled MapType / MapSize / MatchLength / TurnsMode it implies, AND grey-out the standalone Map Size / AI Difficulty controls when picking a mode (the mode IS the bundle). Add an "Advanced" disclosure that re-enables the per-axis overrides for the player who wants to deviate. Today the mode card doesn't say "this picks a Medium balanced board with real-time short matches" — it just sets a key.
