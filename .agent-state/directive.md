...
- [ ] [HIGH] M_TEST.BROWSER.1 — vitest browser harness adoption (user, 2026-05-23: "ive noticed ZERO use of vitest browser plugin youve ONLY been doing unit. look at how other games run headed in real chrome using vitest browser... youve been overly relying on chrome dev tools mcp but look at stuff like ../mean-streets"). Audit pattern: open ../mean-streets/tests/browser/ for examples of how a sibling arcade-cabinet project structures real-chromium tests via @vitest/browser playwright provider. Today's tests/browser/particle-emitter.browser.test.tsx is the ONLY browser test and is shallow. Expand: render visual baselines for every render-touching slice (terrain, FactionBase, Units, particle emitters, modals, the new HiddenBonus + Embers + Snow consumers, ZoneBorder, ContestedPulse, CombatText, BuildQueueStrip) so future visual regressions land RED in CI instead of needing chrome-devtools-mcp probing each time. Each commit that touches src/render/ or src/world/ or src/hud/ should ADD or UPDATE a baseline; the existing commit-gate's pnpm test:browser rule will start to actually bite.
