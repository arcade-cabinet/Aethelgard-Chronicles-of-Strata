...
- [ ] [HIGH] M_TEST.BROWSER.1 — vitest browser harness adoption (user, 2026-05-23). CORRECTION on initial audit: we DO have @vitest/browser + playwright wired (vitest.config.ts browser project, 49 browser tests across e.g. onboarding/audio/keyboard-shortcuts/critical-warning/discoveries/wonder/etc). Today's pnpm test:browser runs 48 passing + 1 preexisting failing (onboarding). What I've been MISSING is the *visual* baseline locks for the gameplay slices my recent commits touched (Terrain, FactionBase, Units, the new bloodSplash/embers/snow consumers, HiddenBonus tile FX). The sibling pattern at ../mean-streets/src/test/render-browser.tsx + ../mean-streets/vitest.browser.config.ts shows the renderInBrowser + settleBrowser helpers + per-screen .browser.test.tsx pattern. Adopt: write tests/browser/aethelgard-gameplay/*.browser.test.tsx with renderInBrowser → spawn a deterministic ?seed=test-seed-zero game → wait for canvas resolve → expect.toHaveScreenshot (or readPixels assert). Baseline lock per visible slice so future drift fires RED in CI instead of needing chrome-devtools-mcp probing each time.
- [ ] [HIGH] M_TEST.BROWSER.2 — port the chrome-devtools-mcp screenshot pattern I've been using (open page → wait_for(text) → take_screenshot → readPixels) into a reusable browser-test helper. The helper should: navigate to a localhost dev URL with a deterministic ?seed=, wait for a marker (e.g. the canvas resolving), take a Vitest browser screenshot, lock the baseline. Lifts the visual ownership rule from "agent runs mcp probes ad-hoc" to "CI catches drift on every push."
- [ ] [HIGH] M_TEST.BROWSER.3 — replace the determinism flake (M_EXPANSION.S.55 patrol introduced Math.random via randomPerimeterTile; tracked under M_EXPANSION.S.55 closure — now uses game.eventRng). Add a browser test that runs an AI-vs-AI session for N seconds and snapshots the world fingerprint (entities + positions + economies); pin determinism end-to-end at the visual layer.
