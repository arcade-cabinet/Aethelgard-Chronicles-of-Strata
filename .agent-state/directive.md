...
- [ ] [HIGH] M_POLISH3.SCENE.4 — PARTIAL. faf4cb1 added window.__triggerGameOver hook that mutates outcome + advances sim frames; journey 06/07 use it. The Dialog still doesn't reliably mount in headless because GameOverModal's rAF poller doesn't fire often enough. Follow-up: replace rAF with a proper state subscription on game.outcome (the koota world might already emit a change event we can hook), OR add a small CustomEvent the trigger dispatches that GameOverModal listens for directly.
