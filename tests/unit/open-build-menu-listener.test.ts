 ...
 * tap. App.tsx listens and selects the player's TownHall, which has
 ...
 * pure-game-state function `selectPlayerTownHall(game)` exists and
...
function selectPlayerTownHall(game: GameState): boolean {
...
describe('M_POLISH2.B.1 — open-build-menu wiring', () => {
  it('selects the player TownHall when called', () => {
    const game = startGame('build-menu-listener');
    const before = game.selectedId;
    const ok = selectPlayerTownHall(game);
    expect(ok).toBe(true);
  ...
  it('returns false when no player TownHall exists (clean game state without bases)', () => {
    const game = startGame('build-menu-no-base');
    // Despawn every player TownHall to simulate the edge case.
    for (const ent of game.world.query(Building, FactionTrait)) {
      const f = ent.get(FactionTrait);
      const b = ent.get(Building);
      if (b?.buildingType === 'TownHall' && f?.faction === 'player') {
