...
/**
 * Max enemies that may (re)acquire a target in a single tick. Retargeting runs
 * a nearest-scan plus an A* path, so it is the costliest AI work; capping it
 * bounds the per-frame cost regardless of how many enemies need a target at
 * once (e.g. a fresh wave). Enemies over the cap simply retarget next tick —
 * a one-frame delay is imperceptible. The M7 yuka rewrite supersedes this with
 * spatial partitioning; until then this keeps the frame budget stable.
 */
const MAX_RETARGETS_PER_TICK = 8;
 ...
 * A* path toward it. An enemy whose target died (or has none) retargets, at
 * most `MAX_RETARGETS_PER_TICK` per tick.
 */
export function aiSystem(world: World, board: BoardData, graph: NavGraph): void {
  ...
  world
    ...
    .updateEach(([faction, target, hex, path]) => {
      ...
      // rate-limit the costly retarget (nearest-scan + A*) per tick
      if (retargets >= MAX_RETARGETS_PER_TICK) return;
      retargets += 1;
